...
def addRepeatingJob(func, delay, first=0):
    ...
    logger.info(__name__, "Repeating job added to queue: {}, repeat delay: {}s, start delay: {}s".format(func.__name__, delay, first))
...
def addSingleJob(func, delay, args):
    ...
    logger.info(__name__, "Single job added to queue: {}, start delay: {}s".format(func.__name__, delay))
